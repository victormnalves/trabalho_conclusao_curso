...
log using "$Logfolder/em_diff-in-diff_cumulativo.log", replace
...
outreg2 using "$output/em_diff_in_diff_cum.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe )  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster ano uf)  

*enem_nota_redacao_std
foreach x in  "enem_nota_redacao_std" {
xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(`x' , cumulativo alguns , fe )  


xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster ano uf )  
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel replace label ctitle(apr_em_std, cumulativo alguns, fe )

xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(apr_em_std, cumulativo alguns, fe cluster uf)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(apr_em_std,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster ano uf )
...
xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(`x', cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 


xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster ano uf ) 
}
...
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel replace label ctitle(enem objetiva PE, cumulativo alguns , fe)  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe  cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel append label ctitle(enem objetiva PE, cumulativo alguns , fe cluster meso)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel append label ctitle(enem objetiva PE, cumulativo alguns , fe cluster ano uf)
...
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles   if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe 
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel append label ctitle(`x' PE, cumulativo alguns, fe) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_PE.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster ano uf)
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel replace label ctitle(apr_em_std PE, cumulativo alguns, fe)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel append label ctitle(apr_em_std PE,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel append label ctitle(apr_em_std PE,  cumulativo alguns, fe cluster ano uf)

*rep_em_std aba_em_std dist_em_std
foreach x in "rep_em_std" "aba_em_std" "dist_em_std"   {
xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26   , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel append label ctitle(`x' PE, cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster meso) 
xtreg  `x' $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_PE.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster ano uf)
