...
log using "$Logfolder/em_placebo.log", replace
...
*enem_nota_objetivo,std
	*fe
	...
	outreg2 using "$output/em_placebo_v1.xls", excel replace label ctitle(enem objetiva,  fe)

	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(enem objetiva,  cluster meso)


	*cluster uf ano
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(enem objetiva,  cluster uf ano)
...
foreach x in "enem_nota_redacao_std"  {	
	...
	*fe
	...
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_nota d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_nota d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_nota d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_placebo_v1.xls", excel append label ctitle(`x',  cluster uf ano)
}
