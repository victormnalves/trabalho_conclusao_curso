...
log using "$Logfolder/em_alavancas_v3.log", replace
...
/*regressões com dummies de alavancas ocm interação*/
	...
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel replace label ctitle(enem objetiva,  fe)

	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $alavancas_nota $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $alavancas_nota $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)
	...
	*cluster uf ano
	...
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster uf ano)
	
	*cluster cidades
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $alavancas_nota $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cidades)
...
foreach x in "enem_nota_redacao_std"  {	
	...
	*fe
	...
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_nota $alavancas_nota  d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_nota $alavancas_nota d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_nota $alavancas_nota d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	*cluster cidade
	xtreg `x' d_ice_nota $alavancas_nota d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_al_notas_v3.xls", excel append label ctitle(`x',  cluster cidades)


}

capture erase "$output/em_diff_in_diff_al_fluxo_v3.xls"
foreach x in "apr_em_std" "rep_em_std" "aba_em_std" "dist_em_std"  {	

	*fe
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_al_fluxo_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	
	*cluster cidade
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster cidades)
...
*enem_nota_objetivo_std
	...
	*fe
	...
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel replace label ctitle(enem objetiva,  fe)

	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_todas $controles $estado if (d_dep4 == 0 | ice == 1), ///
	fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_todas $controles $estado if (d_dep4 == 0 | ice == 1), ///
	fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)


	*cluster uf ano
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_todas $controles $estado if (d_dep4 == 0 | ice == 1), ///
	fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster uf ano)
	
	

	*cluster cidade
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_todas $controles $estado if (d_dep4 == 0 | ice == 1), ///
	fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva, cluster cidade)
	...
	*fe
	...
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva,  fe)
	...
	*cluster cidade
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_algum $controles $estado if (d_dep4 == 0 | ice == 1), ///
	fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster cidade)
...
foreach x in "enem_nota_redacao_std"  {	
	...
	*fe
	xtreg `x' d_ice_nota $al_notas_agregadas_todas d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_nota $al_notas_agregadas_todas  d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_nota $al_notas_agregadas_todas d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_nota $al_notas_agregadas_todas d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  cluster uf ano)
	
	
	*cluster uf ano
	xtreg `x' d_ice_nota $al_notas_agregadas_todas d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  cluster cidade)
	...
	*cluster cidades
	xtreg `x' d_ice_nota $al_notas_agregadas_algum d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_v3.xls", excel append label ctitle(`x',  cidade)
...
capture erase "$output/em_diff_in_diff_al_agr_fluxo_v3.xls"
foreach x in "apr_em_std" "rep_em_std" "aba_em_std" "dist_em_std"  {	

	*fe
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	*cluster cidade
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano* $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_v3.xls", excel append label ctitle(`x',  cluster cidade)
...
Principal components (eigenvectors) 
    ...
    ----------------------------------------------------------------------------------------
*/
* note que pela matriz do pca, os componentes 1 e 2 contabilizam
* 92% da variação
* esse serão os componentes usados
* ainda, como a alavanca projeto de vida não está correlacionada individualmente
* com nenhum desses dois componentes, vamos usar ela sozinha como dummy nas regressões

global componentes_interacao_nota d_ice_nota_comp1 d_ice_nota_comp2 
global componentes_interecao_fluxo d_ice_fluxo_comp1 d_ice_fluxo_comp2 
...
/*regressões com componentes das alavancas*/
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		...
		d_ice_nota_al8 ///
		$controles $estado if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel replace label ctitle(enem objetiva,  fe)

	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		$componentes_interacao_nota d_ice_nota_al8 ///
		$controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		$componentes_interacao_nota d_ice_nota_al8 ///
		$controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)


	*cluster uf ano
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		$componentes_interacao_nota d_ice_nota_al8 ///
		$controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster uf ano)
	...
	*cluster cidade
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		$componentes_interacao_nota d_ice_nota_al8 ///
		$controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(enem objetiva,  cluster cidades)
	

foreach x in "enem_nota_redacao_std"  {	

	*fe
	xtreg `x' d_ice_nota $componentes_interacao_nota d_ice_nota_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_nota $componentes_interacao_nota  d_ice_nota_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_nota $componentes_interacao_nota d_ice_nota_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_nota $componentes_interacao_nota d_ice_nota_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	*cluster cidade
	xtreg `x' d_ice_nota $componentes_interacao_nota d_ice_nota_al8 /// 
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic) 
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_v3.xls", excel append label ctitle(`x',  cluster cidade)
...
*apr_em_std
	*fe
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel replace label ctitle(apr_em_std,  fe)
	
	*cluster codigo_uf
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(apr_em_std,  cluster estado)

	*cluster cod_meso
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(apr_em_std,  cluster meso)

	*cluster uf ano
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(apr_em_std,  cluster uf ano)
	...
	*cluster uf ano
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(apr_em_std,  cluster cidade)


foreach x in  "rep_em_std" "aba_em_std" "dist_em_std"  {	

	*fe
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(`x',  fe)
	
	*cluster codigo_uf
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster estado)

	*cluster cod_meso
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster uf ano)
	...
	*cluster uf ano
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_v3.xls", excel append label ctitle(`x',  cluster cidade)
