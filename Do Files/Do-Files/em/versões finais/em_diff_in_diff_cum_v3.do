...
difference in difference multiple treatment periods
...
log using "$Logfolder/em_diff-in-diff_cumulativo_v3.log", replace
...
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe )  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster ano uf)  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster cidade)  
...
xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe )  


xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster ano uf )  

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster cidade )  
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel replace label ctitle(apr_em_std, cumulativo alguns, fe )

xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(apr_em_std, cumulativo alguns, fe cluster uf)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster ano uf )

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster cidades)
...
xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 


xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster ano uf ) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster cidades)
}
...
xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel replace label ctitle(enem objetiva PE, cumulativo alguns , fe)  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe  cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(enem objetiva PE, cumulativo alguns , fe cluster meso)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(enem objetiva PE, cumulativo alguns , fe cluster ano uf)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(enem objetiva PE, cumulativo alguns , fe cluster cidades)
...
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles   if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe 
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(`x' PE, cumulativo alguns, fe) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster ano uf)

xtreg  `x' $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_PE_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster cidades)
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel replace label ctitle(apr_em_std PE, cumulativo alguns, fe)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(apr_em_std PE,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(apr_em_std PE,  cumulativo alguns, fe cluster ano uf)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(apr_em_std PE,  cumulativo alguns, fe cluster cidades)

*rep_em_std aba_em_std dist_em_std
foreach x in "rep_em_std" "aba_em_std" "dist_em_std"   {
xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26   , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(`x' PE, cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster meso) 
xtreg  `x' $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster ano uf)
xtreg  `x' $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)&codigo_uf==26 , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_PE_v3.xls", excel append label ctitle(`x' PE,  cumulativo alguns, fe cluster cidades)
}
...
xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe )  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster ano uf)  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster cidade)  
...
xtreg  `x' $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe )  


xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster ano uf )  

xtreg  `x' $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_tt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster cidade )  
...
xtreg  apr_em_std  $fluxo_alguns_anos $time_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel replace label ctitle(apr_em_std, cumulativo alguns, fe )

xtreg  apr_em_std  $fluxo_alguns_anos $time_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(apr_em_std, cumulativo alguns, fe cluster uf)

xtreg  apr_em_std  $fluxo_alguns_anos $time_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos $time_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster ano uf )

xtreg  apr_em_std  $fluxo_alguns_anos $time_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster cidades)
...
xtreg  `x'  $fluxo_alguns_anos $time_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos $time_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $fluxo_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 


xtreg  `x'  $fluxo_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster ano uf ) 

xtreg  `x'  $fluxo_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster cidades)
}
...
xtreg  enem_nota_objetivab_std  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe )  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster ano uf)  


xtreg  enem_nota_objetivab_std  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster cidade)  
...
xtreg  `x' $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe )  


xtreg  `x'  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x' $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster ano uf )  

xtreg  `x' $nota_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  ,  fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_ttt_v3.xls", excel append label ctitle(`x' , cumulativo alguns , fe cluster cidade )  
...
xtreg  apr_em_std  $fluxo_alguns_anos $treatment_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel replace label ctitle(apr_em_std, cumulativo alguns, fe )

xtreg  apr_em_std  $fluxo_alguns_anos $treatment_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(apr_em_std, cumulativo alguns, fe cluster uf)

xtreg  apr_em_std  $fluxo_alguns_anos $treatment_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos $treatment_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster ano uf )

xtreg  apr_em_std  $fluxo_alguns_anos $treatment_trends /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(apr_em_std,  cumulativo alguns,  fe cluster cidades)
...
xtreg  `x'  $fluxo_alguns_anos $treatment_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe ) 

xtreg  `x'  $fluxo_alguns_anos $treatment_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $fluxo_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 


xtreg  `x'  $fluxo_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster ano uf ) 

xtreg  `x'  $fluxo_alguns_anos $treatment_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_diff_in_diff_cum_flux_ttt_v3.xls", excel append label ctitle(`x',  cumulativo alguns,  fe cluster cidades)
}
