...
log using "$Logfolder/em_did_par_out_ind_v3.log", replace


/*objetivab*/

use "$folderservidor\dados_EM_par_nota_objetivab_v3.dta", clear
...
*enem_nota_objetivo,std
	*fe 
	...
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel replace label ctitle(enem objetiva,  fe)
	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cluster estado)
	...
	*cluster codigo_uf
	...
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cluster estado boot)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)
	
	*cluster ano uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cluster ano uf)

	*cluster cidade
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cluster cidade)
	
	
	
	xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , ///
	fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", ///
...
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster ano uf)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic) 
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster cidade)
...
*enem_nota_objetivo_std
	*fe 
	...
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel replace label ctitle(enem objetiva,  fe)
	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota  $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)
	
	*cluster ano uf
	xtreg enem_nota_objetivab_std d_ice_nota  $controles $time_trends if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cluster ano uf)

	*cluster cidade
	xtreg enem_nota_objetivab_std d_ice_nota  $controles $time_trends ///
		if (d_dep4 == 0 | ice == 1), fe vce(cluster cod_munic)
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cluster cidade)
	

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster ano uf)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic) 
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster cidade)

	
use "$folderservidor\dados_EM_par_nota_redacao_v3.dta", clear
...
*enem_nota_redacao_std
	*fe 
	...
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem redacao,  fe)	
	*cluster codigo_uf
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem redacao,  cluster estado)
	
	*cluster codigo_uf boot
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado ///
	if (d_dep4 == 0 | ice == 1), fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem redacao,  cluster estado boot)

	*cluster cod_meso
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem redacao,  cluster meso)

	*cluster ano uf
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(enem redacao,   cluster ano uf)
...
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 
...
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster ano uf)


xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster cidade)
...
*enem_nota_redacao_std
	*fe 
	...
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem redacao,  fe)	
	*cluster codigo_uf
	xtreg enem_nota_redacao_std d_ice_nota   $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem redacao,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_redacao_std d_ice_nota   $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem redacao,  cluster meso)

	*cluster ano uf
	xtreg enem_nota_redacao_std d_ice_nota   $controles $time_trends if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(enem redacao,   cluster ano uf)
...
xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe) 

xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster ano uf)


xtreg  `x'  $nota_alguns_anos $time_trends /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster cod_munic)
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster cidade)
...
use "$folderservidor\dados_EM_par_fluxo_`x'_v3.dta", clear
...
*fluxo
	*fe
	...
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster fe)
	*cluster codigo_uf
	xtreg `x'_em_std d_ice_fluxo  d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster estado)
	*cluster codigo_uf boot
	xtreg `x'_em_std d_ice_fluxo  d_ano*  $controles $estado ///
	if (d_dep4 == 0 | ice == 1),  fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster estado boot)
	
	*cluster cod_meso
	xtreg `x'_em_std d_ice_fluxo d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster meso)
	
	*cluster uf ano
	xtreg `x'_em_std d_ice_fluxo d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1),fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	*cluster cidade
	xtreg `x'_em_std d_ice_fluxo d_ano*  $controles $estado ///
	if (d_dep4 == 0 | ice == 1),fe vce(cluster cod_munic) 	
	outreg2 using "$output/em_did_par_out_ind_v3.xls", excel append label ctitle(`x',  cluster cidade)
...
xtreg  `x'_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , ///
	fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf boot) 
...
xtreg  `x'_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster uf ano)
...
outreg2 using "$output/em_did_cum_par_out_ind_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster cidade)
	...
	*fe
	...
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cluster fe)
	*cluster codigo_uf
	xtreg `x'_em_std d_ice_fluxo   $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cluster estado)
	*cluster cod_meso
	xtreg `x'_em_std d_ice_fluxo  $controles $time_trends if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
	
	*cluster uf ano
	xtreg `x'_em_std d_ice_fluxo  $controles $time_trends if (d_dep4 == 0 | ice == 1),fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cluster uf ano)

	*cluster cidade
	xtreg `x'_em_std d_ice_fluxo $controles $time_trends ///
	if (d_dep4 == 0 | ice == 1),fe vce(cluster cod_munic) 	
	outreg2 using "$output/em_did_par_out_ind_tt_v3.xls", excel append label ctitle(`x',  cluster cidade)
...
xtreg  `x'_em_std   $fluxo_alguns_anos $time_trends ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_did_cum_par_out_ind_tt_v3.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster uf ano)
