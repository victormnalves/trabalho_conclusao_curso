...
log using "$Logfolder/em_diff-in-diff_boot_v3.log", replace
...
/*
	*cluster codigo_uf
	...
	outreg2 using "$output/em_did_boot_v3.xls", excel append label ctitle(enem objetiva,  cluster estado boot)
foreach x in "enem_nota_redacao_std"  {		
	*cluster codigo_uf
	xtreg `x' d_ice_nota d_ano*  $controles $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_did_boot_v3.xls", excel append label ctitle(`x',  cluster estado boot)
}
...
xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
outreg2 using "$output/em_did_cum_boot_v3.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  
...
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
outreg2 using "$output/em_did_cum_boot_v3.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe  vce(bootstrap, reps(10) seed(1)) cluster (codigo_uf)
outreg2 using "$output/em_did_cum_boot_v3.xls", excel append label ctitle(apr_em_std, cumulativo alguns, fe cluster uf)
