...
log using "$folderservidor\logfiles/em_maha_output_v3.log", replace
...
use "$folderservidor\dados_EM_14_v3.dta", clear
	...
	*391 escolas tratadas sobraram
	...
	mahapick enem_nota_objetivab_std_lag enem_nota_objetivab_std_lag_2 ///
		...
		genfile($folderservidor\matches_notas.dta) replace  ///
		nummatches(10) matchon(ano) score


use "$folderservidor\matches_notas.dta", clear
keep codigo_escola
sort codigo_escola
...
save "$folderservidor\dados_EM_par_nota_v3.dta", replace
/*
********************************************************************************
fluxo
********************************************************************************
...
use "$folderservidor\dados_EM_14_v3.dta", clear
	...
	*391 escolas tratadas sobraram
	...
	mahapick apr_em_std_lag - dist_em_std_lag_2, ///
		...
		genfile($folderservidor\matches_fluxo.dta) replace  ///
		nummatches(10) matchon(ano) score


use "$folderservidor\matches_fluxo.dta", clear
keep codigo_escola
sort codigo_escola
quietly by codigo_escola:  gen dup = cond(_N==1,0,_n)
sort codigo_escola dup
...
save "$folderservidor\dados_EM_par_fluxo_v3.dta", replace
...
use "$folderservidor\dados_EM_14_v3.dta", clear
	...
	*391 escolas tratadas sobraram
	...
	mahapick enem_nota_`x'_std_lag enem_nota_`x'_std_lag_2 ///
		...
		genfile($folderservidor\matches_notas_`x'.dta) replace  ///
		nummatches(10) matchon(ano) score


use "$folderservidor\matches_notas_`x'.dta", clear
keep codigo_escola
sort codigo_escola
quietly by codigo_escola:  gen dup = cond(_N==1,0,_n)
sort codigo_escola dup
...
save "$folderservidor\dados_EM_par_nota_`x'_v3.dta", replace
}
...
use "$folderservidor\dados_EM_14_v3.dta", clear
	...
	*391 escolas tratadas sobraram
	...
	mahapick `x'_em_std_lag  `x'_em_std_lag_2, ///
		...
		genfile($folderservidor\matches_fluxo_`x'.dta) replace  ///
		nummatches(10) matchon(ano) score


use "$folderservidor\matches_fluxo_`x'.dta", clear
keep codigo_escola
sort codigo_escola
quietly by codigo_escola:  gen dup = cond(_N==1,0,_n)
sort codigo_escola dup
...
save "$folderservidor\dados_EM_par_fluxo_`x'_v3.dta", replace
}
