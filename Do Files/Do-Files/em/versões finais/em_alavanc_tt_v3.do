...
log using "$Logfolder/em_alavancas_tt_v3.log", replace
...
*/
corr al_engaj_gov - al_proj al_outros if ensino_medio ==1 & ano ==2015
...
/*regressões com dummies de alavancas ocm interação*/
	...
	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $alavancas_nota $controles $time_trends $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_al_notas_tt_v3.xls", excel replace label ctitle(enem objetiva,  cluster meso)


foreach x in "enem_nota_redacao_std"  {	

	*cluster cod_meso
	xtreg `x' d_ice_nota $alavancas_nota d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_notas_tt_v3.xls", excel append label ctitle(`x',  cluster meso)



}

capture erase "$output/em_diff_in_diff_al_fluxo_tt_v3.xls"
foreach x in "apr_em_std" "rep_em_std" "aba_em_std" "dist_em_std"  {	


	*cluster cod_meso
	xtreg `x' d_ice_fluxo $alavancas_fluxo d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_fluxo_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
...
*enem_nota_objetivo_std
	...
	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_todas $controles $time_trends $estado if (d_dep4 == 0 | ice == 1), ///
	fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_tt_v3.xls", excel replace label ctitle(enem objetiva,  cluster meso)
	...
	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $al_notas_agregadas_algum $controles $time_trends $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_tt_v3.xls", excel append label ctitle(enem objetiva,  cluster meso)
...
foreach x in "enem_nota_redacao_std"  {	
	...
	*cluster cod_meso
	xtreg `x' d_ice_nota $al_notas_agregadas_todas d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_tt_v3.xls", excel append label ctitle(`x',  cluster meso)


	*cluster cod_meso
	xtreg `x' d_ice_nota $al_notas_agregadas_algum d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_notas_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
...
capture erase "$output/em_diff_in_diff_al_agr_fluxo_tt_v3.xls"
foreach x in "apr_em_std" "rep_em_std" "aba_em_std" "dist_em_std"  {	


	*cluster cod_meso
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_todas d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_tt_v3.xls", excel append label ctitle(`x',  cluster meso)

	*cluster cod_meso
	xtreg `x' d_ice_fluxo $al_fluxo_agregadas_algum d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_al_agr_fluxo_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
...
/*regressões com componentes das alavancas*/
	...
	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* ///
		...
		$controles $time_trends $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_tt_v3.xls", excel replace label ctitle(enem objetiva,  cluster meso)
...
foreach x in "enem_nota_redacao_std"  {	
	...
	*cluster cod_meso
	xtreg `x' d_ice_nota $componentes_interacao_nota d_ice_nota_al8 ///
		d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_notas_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
...
*apr_em_std
	...
	*cluster cod_meso
	xtreg apr_em_std d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_tt_v3.xls", excel replace label ctitle(apr_em_std,  cluster meso)
...
foreach x in  "rep_em_std" "aba_em_std" "dist_em_std"  {	
	...
	*cluster cod_meso
	xtreg `x' d_ice_fluxo $componentes_interecao_fluxo d_ice_fluxo_al8 ///
		d_ano*  $controles $time_trends $estado ///
		if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_comp_al_fluxo_tt_v3.xls", excel append label ctitle(`x',  cluster meso)
