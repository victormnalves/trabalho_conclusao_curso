...
log using "$Logfolder/em_diff-in-diff_par_cens_esc_enem.log", replace

use "$folderservidor\dados_EM_par_cen_esc_enem.dta", clear
...
*enem_nota_objetivo,std
	*fe 
	...
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel replace label ctitle(enem objetiva,  fe)
	*cluster codigo_uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem objetiva,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem objetiva,  cluster meso)
	
	*cluster ano uf
	xtreg enem_nota_objetivab_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem objetiva,  cluster ano uf)
*enem_nota_redacao_std
	*fe 
	xtreg enem_nota_redacao_std d_ice_nota d_ano* $controles $estado if (d_dep4 == 0 | ice == 1), fe 
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem redacao,  fe)	
	*cluster codigo_uf
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem redacao,  cluster estado)

	*cluster cod_meso
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem redacao,  cluster meso)

	*cluster ano uf
	xtreg enem_nota_redacao_std d_ice_nota d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(enem redacao,   cluster ano uf)
...
*fluxo
	*fe
	...
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(`x',  cluster fe)
	*cluster codigo_uf
	xtreg `x' d_ice_fluxo  d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(codigo_uf)
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(`x',  cluster estado)
	*cluster cod_meso
	xtreg `x' d_ice_fluxo d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1), fe cluster(cod_meso)	
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(`x',  cluster meso)

	*cluster uf ano
	xtreg `x' d_ice_fluxo d_ano*  $controles $estado if (d_dep4 == 0 | ice == 1),fe vce(cluster double_cluster) nonest	
	outreg2 using "$output/em_diff_in_diff_par_cen_esc_enem_v1.xls", excel append label ctitle(`x',  cluster uf ano)
}
global estado d_uf*
global outcomes apr_em_std rep_em_std aba_em_std dist_em_std ///
enem_nota_matematica_std  enem_nota_ciencias_std /// 
...
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel replace label ctitle(enem objetiva, cumulativo alguns , fe)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(enem objetiva, cumulativo alguns , fe cluster uf)  

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster meso)

xtreg  enem_nota_objetivab_std  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(enem objetiva,  cumulativo alguns, fe cluster ano uf)

* enem_nota_redacao_std
foreach x in  "enem_nota_redacao_std" {
xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe 
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(`x', cumulativo alguns , fe) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x'  $nota_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_enem.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster ano uf)
...
xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe 
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel replace label ctitle(apr_em_std, cumulativo alguns , fe )


xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(apr_em_std, cumulativo alguns , fe cluster uf)

xtreg  apr_em_std  $fluxo_alguns_anos /// $fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(apr_em_std,  cumulativo alguns, fe cluster meso )

xtreg  apr_em_std  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(apr_em_std, cumulativo alguns , fe cluster uf ano)

*rep_em_std aba_em_std dist_em_std
foreach x in "rep_em_std" "aba_em_std" "dist_em_std"   {
xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe 
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(`x', cumulativo alguns , fe )

xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles  if (d_dep4 == 0 | ice == 1)  , fe cluster(codigo_uf)
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(`x', cumulativo alguns, fe cluster uf) 

xtreg  `x'  $fluxo_alguns_anos /// $nota_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1) , fe cluster(cod_meso)
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(`x',  cumulativo alguns, fe cluster meso) 

xtreg  `x'  $fluxo_alguns_anos ///$fluxo_todos_anos_integral ///
	d_ano* $controles if (d_dep4 == 0 | ice == 1)   , fe vce(cluster double_cluster) nonest
outreg2 using "$output/em_diff_in_diff_cum_enem_flux.xls", excel append label ctitle(`x', cumulativo alguns , fe cluster uf ano)
