...
foreach outcome in "enem_nota_objetivab" {
	*Acumulado
	...
	outreg2 using "$output/ICE_resultados_cum_ps_em_2003_2014.xls", excel append label ctitle(`outcome', 9 anos) 
	...
	*sem cluster
	...
	outreg2 using "$output/ICE_resultados_cum_ps_em_2003_2014.xls", excel append label ctitle(`outcome', 9 anos, pe) 
	...
	*sem cluster
	...
	outreg2 using "$output/ICE_resultados_cum_ps_em_2003_2014.xls", excel append label ctitle(`outcome', 9 anos, sp) 
...
foreach outcome in "apr_em" "rep_em" "aba_em" "dist_em" {
	*Acumulado
	...
	outreg2 using "$output/ICE_resultados_cum_ps_em_2003_2014.xls", ///
...
foreach outcome in "enem_nota_matematica" "enem_nota_ciencias" "enem_nota_humanas" ///
	...
	*------------------------------------------------------
		...
		*/
		...
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', controle pub, tudo) 
		...
		*/	
		...
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', controle pub, integral) 
		...
		*/
		...
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', controle pub) 
		lincom d_ice + d_ice_inte 
		...
		*/
		xtreg  `outcome'_std d_ice d_ano* `controles'  if dep!=4&(d_rigor1==1|ice==0) & ///
			(integral==1|ice==0), fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', apoio forte)

		xtreg  `outcome'_std d_ice d_ano* `controles'  if dep!=4&(d_rigor3==1|ice==0) & ///
			(integral==1|ice==0), fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', apoio medio)

		xtreg  `outcome'_std d_ice d_ano* `controles'  if dep!=4&(d_rigor2==1|ice==0) & ///
			(integral==1|ice==0), fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', apoio fraco)

		xtreg  `outcome'_std d_ice d_ano* `controles'  if dep!=4&(d_rigor4==1|ice==0) & ///
			(integral==1|ice==0), fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', sem apoio)

		xtreg  `outcome'_std d_ice d_ano* `controles'  if dep!=4&(d_rigor4==0|ice==0) & ///
			(integral==1|ice==0) , fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", excel append label ctitle(`outcome', com algum apoio)
}
...
foreach outcome in   "enem_nota_objetivab"{
		...
		*/
		foreach uf in 23 26 35 52 {
			...
			outreg2 using "$output/ICE_resultados_em_`uf'_2003_2014.xls", excel append label ctitle(`outcome') 
		}
		...
		*/
		xtreg `outcome'26_std d_ice d_ice_inte d_ano* `controles'  if dep != 4 & (uf == "PE") & ///
				(ano_ice==2008|ano_ice==2009|ano_ice==2010) , fe 
		outreg2 using "$output/ICE_resultados_em_PE_semi_int_2003_2014.xls", excel append label ctitle(`outcome') 
		...
		*/
		xtreg `outcome'2326_std d_ice d_ice_inte d_ano* `controles'  if dep != 4 & ///
			(uf=="PE"|uf=="CE")&(ano_ice==2008|ano_ice==2009|ano_ice==2010) , fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_PE_CE_semi_int_2003_2014.xls", excel append label ctitle(`outcome') 
		...
		*/
		xtreg `outcome'2326_std d_ice  d_ano* `controles'  if dep != 4 & ///
			(uf=="PE"|uf=="CE") , fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_PE_CE_2003_2014.xls", excel append label ctitle(`outcome') 
		...
		*/
		...
		xtreg `outcome'3552_std d_ice  d_ano* `controles'  if dep !=4 & ///
			(uf=="GO"|uf=="SP") , fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_SP_GO_2003_2014.xls", excel append label ctitle(`outcome') 
...
foreach outcome in "apr_em" "rep_em" "aba_em" "dist_em" {
		...
		*/
		xtreg  `outcome'_std d_ice d_ano* `controles'  ///
			if dep!=4, fe cluster(codigo_uf)
		outreg2 using "$output/ICE_resultados_em_2003_2014.xls", ///
			excel append label ctitle(`outcome', controle pub, tudo) 
...
foreach outcome in "apr_em" "rep_em" "aba_em" "dist_em" {
	...
	*/
		foreach xx in 23 26 35 52 {
			xtreg `outcome'`xx'_std d_ice d_ano* `controles'  ///
				if dep!=4  & codigo_uf==`xx', fe 
			outreg2 using "$output/ICE_resultados_em_`xx'_2003_2014.xls", ///
				excel append label ctitle(`outcome') 
		}
	*Interegral vs Semi PE 
	/*
	...
	*/
	...
	xtreg `outcome'26_std d_ice d_ice_inte d_ano* `controles' /// 
		 if dep != 4 & (uf=="PE") & ///
		(ano_ice==2008|ano_ice==2009|ano_ice==2010) , fe 
	outreg2 using "$output/ICE_resultados_em_PE_semi_int_2003_2014.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	xtreg `outcome'2326_std d_ice d_ice_inte d_ano* `controles' ///
		...
		(ano_ice==2008|ano_ice==2009|ano_ice==2010) , fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_em_PE_CE_semi_int_2003_2014.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	xtreg `outcome'2326_std d_ice  d_ano* `controles' ///
		 if dep!=4&(uf=="PE"|uf=="CE") , ///
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_em_PE_CE_2003_2014.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	xtreg `outcome'3552_std d_ice  d_ano* `controles' ///
		...
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_em_SP_GO_2003_2014.xls", ///
		excel append label ctitle(`outcome') 
