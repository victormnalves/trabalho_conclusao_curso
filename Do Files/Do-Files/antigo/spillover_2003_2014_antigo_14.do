...
global output "\\fs-eesp-01\EESP\Usuarios\leonardo.kawahara\resultados_spillover"
...
log using "$Logfolder/spillover_antigo.log", replace
...
collapse (mean) ice codigo_uf taxa_participacao_enem 					/// 
	...
	n_alunos_em_ep apr_em rep_em aba_em dist_em apr_ef rep_ef aba_ef 		///
	dist_ef, 						///
...
foreach x in "enem_nota_matematica_std" "enem_nota_ciencias_std" 		///
	...
	***Controlando por demais carateristicas da escola 
	...
	outreg2 using "$output/ICE_resultados_spillover_uf_ps_antigo.xls", 		///
...
collapse (mean) ice codigo_uf taxa_participacao_pb pb_esc_sup_mae 		///
	pb_esc_sup_pai media_lp_prova_brasil_9 media_mt_prova_brasil_9		///
	...
	dist_em apr_ef rep_ef aba_ef 		///
	dist_ef, by(codigo_municipio_novo ano)
...
*pscore ice n_alunos_ef taxa_participacao_pb   if ano==2011&codigo_uf==32, pscore(pscores_es)
...
foreach x in  "media_lp_prova_brasil_9_std" 						/// 
	...
	outreg2 using "$output/ICE_resultados_spillover_ef_ps_2003_2014_antigo.xls",			///
...
outreg2 using "${output}ICE_resultados_spillover_ps_uf_antigo.xls", excel append label ctitle(`x', `xx') 
...
foreach x in  "apr_ef" "rep_ef" "aba_ef" "dist_ef"  {
	...
	xtreg `x' m_tem_ice d_ano* pb_esc_sup_mae pb_esc_sup_pai				///
		...
		lab_ciencias quadra_esportes biblioteca internet [pw=pscore_total], ///
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_spillover_ps_ef_2003_2014_antigo.xls",			///
		excel append label ctitle(`x', escola2) 
...
foreach x in "enem_nota_matematica_std" "enem_nota_ciencias_std" 		///
	...
	***Controlando por demais carateristicas da escola 
	xtreg `x'_`xx' m_tem_ice d_ano* e_escol_sup_pai e_escol_sup_mae 	/// 
		...
		n_mulheres_ef_em_ep n_brancos_ef_em_ep rural agua eletricidade  /// 
		esgoto lixo_coleta sala_professores lab_info lab_ciencias       ///
		quadra_esportes biblioteca   internet, fe 
	outreg2 using "$output/ICE_resultados_spillover_uf_antigo.xls", 		///
		excel append label ctitle(`x', `xx') 
...
foreach x in  "media_lp_prova_brasil_9_std" 						/// 
	...
	xtreg `x' m_tem_ice d_ano* pb_esc_sup_mae pb_esc_sup_pai				///
		...
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_spillover_ef_2003_2014_antigo.xls", excel append label ctitle(`x', `xx') 
...
foreach x in  "apr_ef" "rep_ef" "aba_ef" "dist_ef"  {
	...
	xtreg `x' m_tem_ice d_ano* pb_esc_sup_mae pb_esc_sup_pai				///
		...
		lab_ciencias quadra_esportes biblioteca internet, ///
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_spillover_ef_2003_2014_antigo.xls",			///
		excel append label ctitle(`x', escola2) 
