...
log using "$Logfolder/ef_diff_in_diff_cum_v3.log", replace
...
*media_lp_prova_brasil_9_std
	...
	outreg2 using "$output/ef_diff_in_diff_cum_v3.xls", excel replace label ///
		 ctitle(pb port,  fe)
	...
	outreg2 using "$output/ef_diff_in_diff_cum_v3.xls", excel append label ///
...
foreach x in  "media_mt_prova_brasil_9_std" "media_pb_9_std" "apr_ef_std" "rep_ef_std" "aba_ef_std" "dist_ef_std"  {	 
	xtreg `x' $todos_anos d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe
	outreg2 using "$output/ef_diff_in_diff_cum_v3.xls", ///
		excel append label ///
		ctitle(`x',  fe)
