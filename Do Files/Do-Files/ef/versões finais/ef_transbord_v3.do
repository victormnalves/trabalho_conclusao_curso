...
/*
Efeito transbordamento na base de Ensino Fundamental
...
log using "$Logfolder/ef_transbord_v3.log", replace
...
*media_lp_prova_brasil_9_std
	...
	outreg2 using "$output/ef_diff_in_diff_placebo_v3.xls", excel replace label ///
		ctitle(pb port,  fe)
	xtreg media_lp_prova_brasil_9_std d_ice  d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe cluster(codigo_uf)
	outreg2 using "$output/ef_diff_in_diff_placebo_v3.xls", excel append label ///
...
foreach x in "media_mt_prova_brasil_9_std" "media_pb_9_std" "apr_ef_std" "rep_ef_std" "aba_ef_std" "dist_ef_std"  {	 
	xtreg `x' d_ice d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe 
	outreg2 using "$output/ef_diff_in_diff_placebo_v3.xls", ///
		excel append label ///
		ctitle(`x',  fe)
	xtreg `x' d_ice d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe cluster(codigo_uf)
