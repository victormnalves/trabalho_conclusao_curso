...
log using "$Logfolder/ef_diff_in_diff_par_pb_cens_esc.log", replace
use "$folderservidor\dados_EF_par_cen_esc_pb.dta", clear
...
*media_lp_prova_brasil_9_std
	...
	outreg2 using "$output/ef_diff_in_diff_par_pb_censo_esc.xls", excel replace label ///
		 ctitle(pb port,  fe)
	xtreg media_lp_prova_brasil_9_std d_ice  d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe cluster(codigo_uf)
	outreg2 using "$output/ef_diff_in_diff_par_pb_censo_esc.xls", excel append label ///
	...
	outreg2 using "$output/ef_diff_in_diff_par_pb_censo_esc.xls", excel append label ///
		 ctitle(pb port,  cluster ano uf)
*fluxo 
	*media_mt_prova_brasil_9_std media_pb_9_std apr_ef_std rep_ef_std aba_ef_std dist_ef_std
foreach x in "media_mt_prova_brasil_9_std" "media_pb_9_std" "apr_ef_std" "rep_ef_std" "aba_ef_std" "dist_ef_std"   {	 
	...
	xtreg `x' d_ice d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe cluster(codigo_uf)
	outreg2 using "$output/ef_diff_in_diff_par_pb_censo_esc.xls", ///
		excel append label ///
...
*media_lp_prova_brasil_9_std
	...
	d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe 
	outreg2 using "$output/ef_diff_in_diff_cum_par_pb_censo_esc.xls", excel replace label ///
		 ctitle(pb port,  fe)
	...
	d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe cluster(codigo_uf)
	outreg2 using "$output/ef_diff_in_diff_cum_par_pb_censo_esc.xls", excel append label ///
		 ctitle(pb port,  cluster estado)
...
foreach x in  "media_mt_prova_brasil_9_std" "media_pb_9_std" "apr_ef_std" "rep_em_std" "aba_em_std" "dist_em_std"   {	 
	xtreg `x' $todos_anos d_ano* $controles $estado ///
		if (d_dep4==0|ice==1), fe 
	outreg2 using "$output/ef_diff_in_diff_cum_par_pb_censo_esc.xls", ///
		excel append label ///
		ctitle(`x',  fe)
