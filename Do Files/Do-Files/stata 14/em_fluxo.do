...
********************************************************************************
/*------------------------------------------------------------------------------
a base de dados para análise de impacto em nota é ice_clean_fluxo.dta
*/
...
*********************************** 2b. Interações Turno e Alavanca ***********************
/*dummy ice integral*/
gen ice_inte=0
...
d_ice_inte assumi um no ano de entrada da escola integral
*/
gen d_ice_inte=d_ice*ice_inte

/*dummy interação ice e semi-integral
d_ice_semi_inte assumi um no ano de entrada da escola integral
*/
...
lab_ciencias quadra_esportes biblioteca internet
	...
	*/
	foreach xx in 23 26 35 52 {
		...
		outreg2 using "$output/ICE_resultados_ps_em_`xx'_new.xls", ///
	...
	*/
	...
	outreg2 using "$output/ICE_resultados_ps_em_PE_semi_int.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	...
	outreg2 using "$output/ICE_resultados_ps_em_PE_CE_semi_int.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	xtreg `outcome'2326_std d_ice  d_ano* `controles' ///
		[pw=pscore_total] if dep!=4&(uf=="PE"|uf=="CE") , ///
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_ps_em_PE_CE.xls", ///
		excel append label ctitle(`outcome') 
	...
	*/
	xtreg `outcome'3552_std d_ice  d_ano* `controles' ///
		...
		fe cluster(codigo_uf)
	outreg2 using "$output/ICE_resultados_ps_em_SP_GO.xls", ///
		excel append label ctitle(`outcome') 

}
*********************************** 2f. Estimações e resultados ***************** 
...
foreach outcome in "apr_em" "rep_em" "aba_em" "dist_em" {
	*Acumulado
	...
	outreg2 using "$output/ICE_resultados_cum_em_new.xls", ///
